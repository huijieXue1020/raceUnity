...
public class VehicleControl : MonoBehaviour
{
        ...
        if (activeControl)
        {
            ...
            if (controlMode == ControlMode.simple)
            {
                ...
                if (carWheels.wheels.frontWheelDrive || carWheels.wheels.backWheelDrive)
                {
                    ...
                    brake = Input.GetButton("Fire3");
                    //Debug.Log("brake:" + brake);
                    //shift = Input.GetKey(KeyCode.LeftShift) | Input.GetKey(KeyCode.RightShift);
                    shift = Input.GetButton("Fire1");
    ...
    : slip = Mathf.Lerp(slip, 0.01f, 0.02f);
            ...
            if (shift && (currentGear > 1 && speed > 50.0f) && shifmotor && Mathf.Abs(steer) < 0.2f)
            {
                ...
                powerShift = Mathf.MoveTowards(powerShift, 0.0f, Time.deltaTime * 5.0f);
            ...
            else
            {
                ...
                if (Input.GetButton("Fire3"))  //集气
                {
                    powerShift = Mathf.MoveTowards(powerShift, 100.0f, Time.deltaTime * 10.0f);
                }
